...
This is part of the synthetic experiments, where we predict decays using the ego
network predictor.
"""
...
for noise in noises:
    ...
    for params in paramsList:
        if params[0] == "apgl.predictors.PrimalRidgeRegression":
            paramFunction = getattr(alterRegressor, params[1])
            paramFunction(params[2])

        if params[0] == "apgl.predictors.KernelShiftRegression":
            paramFunction = getattr(egoRegressor, params[1])
            paramFunction(params[2])

    logging.info(edgeLearner)
